...
class CifarTenDataset(GEORGEDataset):
    ...
    def _load_samples(self):
        ...
        if self.ontology == 'four-comp':
            ...
            superclass_labels = torch.tensor(list(map(lambda x : self.class_div.get(int(x),2),original_labels))).long()
            print(np.bincount(superclass_labels))
        else:
...
def get_transform_CIFAR(resize=True, augment=False):
    ...
    if resize:
        test_transform_list.insert(0, transforms.Resize((518, 518)))
    if not augment:
